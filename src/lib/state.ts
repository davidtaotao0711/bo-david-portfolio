...
import { stripBase, withBase } from './base-path';
...
export function parseRoute(url: URL, projects: Project[], base = '/'): Route {
  const path = stripBase(url.pathname, base).replace(/^\/+|\/+$/g, '');
...
export function projectUrl(slug: string, index = 0, base = '/') { return withBase(`/${slug}?s=${index}`, base); }
